...
import {StrategiesBacktestForm} from "@/src/sections/strategies-graph-test2/strategies.backtest-form";
import {useCallback, useEffect, useMemo, useState} from "react";
import {onSubmitWrapper} from "@/src/utils/submit";
import {useDeleteAllTestMutation, useGetAllTest2Query, useRun2Mutation} from "@/lib/redux/api/dhmApi";
import Button from "@mui/material/Button";
import {camelCase} from "lodash";

export function StrategiesDhmBacktestDialog({ pairId, tf, klines }: any) {
  const [chart, setChart] = useState<any>(null);
  const [run, { isLoading }] = useRun2Mutation();
  const [removeAllTest, { isLoading: isRemovalAllLoading }] = useDeleteAllTestMutation();
  const { data: testDhm } = useGetAllTest2Query({ pairId, tf });
  ...
  const data = useMemo(() => {
    console.log('testDhm?.length', testDhm);
