...
import {StrategiesSettingsForm} from "@/src/sections/strategies-graph2/strategies.settings-form";
import {useGetSettingsDhm2ByPairIdAndTfQuery, useUpdateSettingsDhm2Mutation} from "@/lib/redux/api/dhmApi";
import {useCallback, useEffect} from "react";
import {onSubmitWrapper} from "@/src/utils/submit";
import {useForm} from "react-hook-form";

export function StrategiesDhmSettingsDialog({ tf, pairId }: any) {
  const { data: dhmSettings } = useGetSettingsDhm2ByPairIdAndTfQuery({ tf, pairId });
  const [update, { isLoading }] = useUpdateSettingsDhm2Mutation();
